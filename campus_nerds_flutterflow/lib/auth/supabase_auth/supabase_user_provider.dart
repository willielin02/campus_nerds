...
class CampusNerds2SupabaseUser extends BaseAuthUser {
  CampusNerds2SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> campusNerds2SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CampusNerds2SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
