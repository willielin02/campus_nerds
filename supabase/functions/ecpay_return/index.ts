...
Deno.serve(async (req)=>{
  ...
  if (receivedMac !== expectedMac) return new Response('0|FAIL', {
    status: 200
  });
  ...
  const { data: order, error: orderErr } = await supabaseAdmin.from('orders').select('id,user_id,status,ticket_type_snapshot,pack_size_snapshot').eq('merchant_trade_no', merchantTradeNo).single();
  if (orderErr || !order) return new Response('0|FAIL', {
    status: 200
  });
